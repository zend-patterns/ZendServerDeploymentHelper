...
class Template implements EventManagerAwareInterface, LogAwareInterface{
    ...
    private function isFile($origFilename) {
        ...
        if ($origFilename[0] != '/') $filenameOptions[] = SCRIPT_ROOT . '/' . $origFilename;
    ...
    private function getTemplateContent($filename) {
        ...
        if (!$filename = $this->isFile($filename)) {
            $this->getEventManager()->trigger(__FUNCTION__, $this, array('msg' => "Template-File [$filename] doesn't exist."));
            return false;
        }
    ...
    private function writeToTarget($filename, $data) {
        if (!@file_put_contents($filename, $data)) {
            $this->getEventManager()->trigger(__FUNCTION__, $this, array('msg' => "Target-File [$filename] is not writable."));
            return;
        }
    }
    ...
    public function dryRun($from, $search = null, $replace = null, $writeToLog = true) {
        $content = $this->getTemplateContent($from);
        $content = str_replace($search, $replace, $content);
        if ($writeToLog) $this->log->info('Content from template file after substitution: ' . $content);
